...
vcf_org <- read.csv(file = "data/popgenome_pi_dxy_est.csv")
vcf_files <- list.files("../simulating-test-data", recursive = TRUE, pattern = ".vcf*", full.names = TRUE)
reruns <- vcf_files[!(vcf_files %in% vcf_org$vcf_file)]
reruns <- reruns[!(grepl("/1/", reruns))]
reruns <- reruns[!(grepl("/10000/", reruns))]
reruns <- reruns[!(grepl("/0/", reruns))]
reruns <- reruns[!(grepl("var_only", reruns))]
#reruns <- sample(reruns)

popgenome_df <- lapply(reruns, compute_pi_pop_genome)
#popgenome_df <- mclapply(reruns, compute_pi_pop_genome, mc.cores = 2)
